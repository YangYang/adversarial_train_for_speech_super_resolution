...
def validation_pesq(net, feature_creator):
    ...
    for i in range(100):
        ...
        sf.write('clean_16k_lr_1e-3.wav', clean, sr)
        # 低频部分的语音，最后cat在一起用
        ...
        sf.write('res_16k_lr_1e-3.wav', res.numpy().squeeze(), sr)
        p2 = pesq('clean_16k_lr_1e-3.wav', 'res_16k_lr_1e-3.wav', is_current_file=True)
...
def train(g_net, d_net, g_opt, d_opt, epoch, data_loader, loss_helper):
    global global_step
    path_dir = 'TIMIT_16k_train_learn_speech_with_sn_lsd_no_pre_train_d_in_norm_lr_1e-3/'
...
if __name__ == "__main__":
    ...
    if train_tag:
        ...
        # TODO LR
        generator_opt = optim.Adam(generator.parameters(), lr=1e-3)
        ...
        # discriminator.train()
        discriminator = discriminator.cuda(CUDA_ID[0])
        discriminator_opt = optim.RMSprop(discriminator.parameters(), lr=1e-3)
